...
public class ErosionBrushEditor : Layout
{
	...
	public override void OnInspectorGUI () 
	{
		...
		if (preset.isNoise)
		{
			...
			if (script.guiShowErosion)
			{
				...
				if (preset.noise_seed != tempSeed) 
				{ 
					//Noise.seed = tempSeed; 
					preset.noise_seed = tempSeed; 
					UnityEngine.Random.InitState(tempSeed); //UnityEngine.Random.seed = tempSeed; 
				}
		...
		if (script.guiShowTextures)
		{
			...
			if (script.terrains.Length != 0)
			{
				...
				TerrainLayer[] layers = script.terrains[0].terrainData.terrainLayers;
				Texture2D[] textures = new Texture2D[layers.Length];
				for (int i=0; i<layers.Length; i++) textures[i] = layers[i].diffuseTexture;
				#else
		...
		if (script.guiShowAbout)
		{
			...
			Par(); Label("Erosion Brush v1.5.3");
	...
	public void OnSceneGUI ()
	{
		...
		DrawBrush(brushPos, preset.brushSize*preset.brushFallof, script.terrains, color:script.guiBrushColor/2, thickness:script.guiBrushThickness, numCorners:script.guiBrushNumCorners);
